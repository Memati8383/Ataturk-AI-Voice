...
# --- Yapılandırma ---
MODEL_PATH = "mustafa-kemal-ataturk_1380e_42780s.pth" 
INDEX_PATH = "added_mustafa-kemal-ataturk_v2.index"
...
def convert_voice(input_path, output_path):
    if rvc_model:
        try:
            ...
            print(f"RVC Dönüştürme Hatası: {e}")
            return False
    return False
...
def load_rvc_model():
    ...
    if not rvc_model and RVCInference:
        print("RVC Modeli yükleniyor...")
        try:
            rvc_model = RVCInference(device="cpu:0")
...
async def speak_endpoint(request: SpeakRequest):
    try:
        ...
        if rvc_model:
            conversion_success = await asyncio.to_thread(convert_voice, temp_audio, final_audio)
...
async def get_audio():
    ...
    return HTTPException(status_code=404, detail="Ses dosyası bulunamadı")
